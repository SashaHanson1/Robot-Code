...
MSE 2202 MSEBot base code for Lab 3 - exercise
Language: Arduino
...
int pauseTime;                                                                  // used to determine when to pause the robots motion for a moment after return to its start position
...
long disCounts;                                                                // number of encoder pulses for target distance (50cm or 25cm)
long stopLength;                                                               // indcated length of time for which the movement will be stopped (4 or 2 seconds)
            ...
            if (timeUp3sec) {                                                  // pause for 3 sec before running case 1 code
              ...
              if (motorsEnabled) {                                        // run motors only if enabled
                if (timeUp2sec) { 
                  timeUp2sec = false;                                       // reset 2 second timer
                  if (pauseTime == 4) {
                    int pauseLength = millis();
                    while (millis()<50){}
                    pauseTime = 0;
                  }
                  // Drive 50cm in current direction
                  encoder[0].pos = 0;
                  disCounts = 50 * cCountsRev / (4 * pi);
                  while (encoder[0].pos < disCounts) {
                  ...
                  // turn 90 degrees
                  encoder[0].pos = 0;
                  disCounts = ((((17.5 * pi) / 4))/ 4 * pi) * cCountsRev;  // calculate the distance a wheel must travel to turn 90 degrees (measured in encoder pulses)
                  while (encoder[0].pos < disCounts) {
                    setMotor(1, driveSpeed, cIN1Chan[0], cIN2Chan[0]); // left motor forward
                    setMotor(1, driveSpeed, cIN1Chan[1], cIN2Chan[1]);  // right motor forward (opposite orientation)
                  }
                  pauseTime++;
                }
	            }
